...
impl SPriceModelTest {
    pub fn new(period: f64, amplitude: Decimal, origin: DateTime<Local>, mean: Decimal) -> Self {
        Self {
            period,
            amplitude,
            origin,
...
impl TPriceModel for SPriceModelTest {
...
mod tests {
    ...
    pub fn test1() {
        ...
        // 构建模型
        let model = SPriceModelTest::new(period, amplitude, origin, mean);
        ...
        // 测试数据
        // sin30’=0.5
        let time = origin + Duration::seconds((period * 30.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap();
        assert_eq!(value, amplitude * Decimal::from_f64(0.5).unwrap() + mean);
        // sin90’=1.0
        let time = origin + Duration::seconds((period * 90.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap();
        assert_eq!(value, amplitude * Decimal::from_f64(1.0).unwrap() + mean);
        // sin150’=0.5
        let time = origin + Duration::seconds((period * 150.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap();
        assert_eq!(value, amplitude * Decimal::from_f64(0.5).unwrap() + mean);
        // sin180’=0
        let time = origin + Duration::seconds((period * 180.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap().trunc_with_scale(4);
        assert_eq!(value, amplitude * Decimal::from_f64(0.0).unwrap() + mean);
        // sin210’=-0.5
        let time = origin + Duration::seconds((period * 210.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap().trunc_with_scale(4);
        assert_eq!(value, amplitude * Decimal::from_f64(-0.5).unwrap() + mean);
        // sin270’=-1.0
        let time = origin + Duration::seconds((period * 270.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap().trunc_with_scale(4);
        assert_eq!(value, amplitude * Decimal::from_f64(-1.0).unwrap() + mean);
        // sin330’=-0.5
        let time = origin + Duration::seconds((period * 330.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap().trunc_with_scale(4);
        assert_eq!(value, amplitude * Decimal::from_f64(-0.5).unwrap() + mean);
        // sin360’=0
        let time = origin + Duration::seconds((period * 360.0 / 360.0) as i64);
        let value = model.get_price(time).unwrap().trunc_with_scale(4);
        assert_eq!(value, amplitude * Decimal::from_f64(0.0).unwrap() + mean);
    }
}