...
- **做空问题：做空时无法收敛 反馈控制失效！**
  ...
  - [x] debug - ~~做空时平仓单过多 and 平空单价格高于开空单 导致实际仓位无法收敛至目标仓位~~
    - 原因是做空并且仓位过低时，只能等待价格下降来提升仓位，无法挂买单来等待价格下降（如果卖单价格过低会导致直接成交，进一步降低仓位）。
    - 不是bug，是正常现象。
 - [x] 优化script逻辑 充分利用泛型
 - [x] PID逻辑选择：**由于决策时间间隔（1分钟）足够短，直接使用下列策略1.**
   ...
   2. 使用soft目标仓位作为第一个挂单的目标仓位，随后的每一个挂单，需要考虑上个挂单完成后的实际仓位，再使用PID生成新的soft目标仓位。
      - 仓位变化较快
 ~~- [ ] 做空时如何计算实际仓位~~
### 20250605 - Mk_Beta_5设计中
 - [ ] 杠杆资产开发
   - 所有合约标的物，均视为杠杆资产。根据合约方向，做多和做空视为两类不同的资产。
     - 杠杆率>0时为做多，杠杆率<0时为做空。
   - 当杠杆率为+1时，USDT合约可以退化为现货。
     - 区别：现货没有资金费率，而USDT合约有资金费率。
   - 做空合约资产
     - 将合约做空打包成金融资产，做空BTC等于做多该资产，从而保证反馈控制时能够收敛。
     - 需要提前给定杠杆率，举例如下：
       - 如果杠杆率为-2，在100,000价位做空1 BTC，那么需要200,000 USDT作为保证金。
       - 等价于买入200,000 USDT等值的资产：
         - 当BTC下跌50%时，该资产增值50,000 USDT，即上涨25%。
         - 当BTC上涨50%时，该资产缩水50,000 USDT，即下跌25%。
   - [ ] Runner代码改造
   - [ ] StrategyOrder 代码改造
   - [ ] Strategy代码改造：-1倍比特币U本位合约资产 反馈控制
 - 短周期趋势预测算法设计
   - [ ] ~~简易算法：离散傅里叶变换->相位偏移180°->傅里叶逆变换~~
     - 没啥用...
   - [ ] 进阶趋势算法：小波变换+频率重心统计
     - 如果波动集中在高频段，则判断为震荡趋势，提高仓位波动幅度。
     - 如果波动集中在低频段，则判断为单边趋势，降低仓位波动幅度。